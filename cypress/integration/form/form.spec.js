...
context('Form', () => {
    beforeEach(() => {
        ...
        cy.wait(['@postInfos']).its('status').should('be', 200)
    })
...
function closeAnalysisDialog(close){
    if(close){
        cy.get('[data-cy=analysis-dialog-close]')
        .click()
    }
}

function accessDiagnosticsMenuTab(){
    cy.wait(['@getRecommendations']).its('status').should('be', 200)
...
function startRoutes(){
    ...
    cy.route({
        method: 'GET',
        url: 'https://test-api.jdivorce.com/api/lawyers/recommendations/plan?workflow_type=DivorceWorkflow&workflow_stage=diagnostics'
